...
enter waiting, id: 2
back in main thread
wait failed in main thread: busy
still in main thread
wait success, id: 1
wait success, id: 2
...
=== TEST 5: semaphore new not allowed in init_by_lua
--- http_config
...
=== TEST 7: semaphore in set_by_lua (wait is not allowed)
--- http_config eval: $::HttpConfig
--- config
    location /t {
...
=== TEST 8: semaphore in rewrite_by_lua (all allowed)
--- http_config eval: $::HttpConfig
--- config
    location /t {
...
=== TEST 9: semaphore in access_by_lua (all allowed)
--- http_config eval: $::HttpConfig
--- config
    location /t {
        access_by_lua_block {
...
=== TEST 10: semaphore in content_by_lua (all allowed)
--- http_config eval: $::HttpConfig
--- config
    location /t {
        content_by_lua_block {
...
=== TEST 11: semaphore in log_by_lua (wait not allowed)
--- http_config eval: $::HttpConfig
--- config
    location /t {
        echo "ok";
...
=== TEST 12: semaphore in header_filter_by_lua (wait not allowed)
--- http_config eval: $::HttpConfig
--- config
    location /t {
        echo "ok";
...
=== TEST 13: semaphore in body_filter_by_lua (wait not allowed)
--- http_config eval: $::HttpConfig
--- config
    location /t {
        echo "ok";
...
=== TEST 14: semaphore in ngx.timer (all allowed)
--- http_config eval: $::HttpConfig
--- config
    location /t {
        content_by_lua_block {
...
=== TEST 15: semaphore post in all phase (in a request)
--- http_config
    lua_package_path $TEST_NGINX_LUA_PACKAGE_PATH;
    init_worker_by_lua_block {
        local semaphore = require "ngx.semaphore"
...
=== TEST 16: semaphore wait post in access_by_lua
--- http_config eval: $::HttpConfig
--- config
    location /test {
        access_by_lua_block {
...
=== TEST 17: semaphore wait post in rewrite_by_lua
--- http_config eval: $::HttpConfig
--- config
    location /t {
        rewrite_by_lua_block {
...
=== TEST 18: semaphore wait in timer.at
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 19: semaphore post in header_filter_by_lua (subrequest)
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 20: semaphore post in body_filter_by_lua (subrequest)
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 21: semaphore post in set_by_lua
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 22: semaphore post in timer.at
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 23: two thread wait for each other
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 24: kill a light thread that is waiting on a semaphore(no resource)
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 25: kill a light thread that is waiting on a semaphore(after post)
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 26: kill a thread that is waiting on another thread that is waiting on semaphore
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 27: a light thread that is going to exit is waiting on a semaphore
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 28: main thread wait a light thread that is waiting on a semaphore
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 29: multi wait and mult post with one semaphore
--- http_config eval: $::HttpConfig
--- config
    location = /test {
        content_by_lua_block {
...
=== TEST 30: semaphore wait time is zero
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
...
=== TEST 31: semaphore wait time default is zero
--- http_config eval: $::HttpConfig
--- config
    location /test {
        content_by_lua_block {
