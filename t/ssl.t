...
=== TEST 23: read SNI name via ssl.client_server_name()
--- skip_openssl: 6: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
        local ssl = require "ngx.ssl"
        print("read SNI name from Lua: ", ssl.client_server_name())
    }
...
=== TEST 24: read SNI name via ssl.client_server_name() when no SNI name specified
--- skip_openssl: 6: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
...
=== TEST 25: read SNI name via ssl.client_server_name() and ssl.server_name()
--- skip_openssl: 10: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
        local ssl = require "ngx.ssl"
        print("ssl client hello read SNI name from client_server_name: ", ssl.client_server_name())
        print("ssl client hello read SNI name from server_name: ", ssl.server_name())
    }
    server {
        listen unix:$TEST_NGINX_HTML_DIR/nginx.sock ssl;
        server_name   test.com;
        ssl_certificate_by_lua_block {
            local ssl = require "ngx.ssl"
            print("ssl cert read SNI name from client_server_name: ", ssl.client_server_name())
            print("ssl cert read SNI name from server_name: ", ssl.server_name())
        }
        ssl_certificate ../../cert/test.crt;
        ssl_certificate_key ../../cert/test.key;
...
--- error_log
lua ssl server name: "test.com"
ssl client hello read SNI name from client_server_name: test.com
ssl client hello read SNI name from server_name: nil
lua ssl server name: "test.com"
ssl cert read SNI name from client_server_name: nil
ssl cert read SNI name from server_name: test.com
...
=== TEST 26: set_protocols - TLSv1
--- skip_openssl: 6: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
        local ssl = require "ngx.ssl"
        local ops = {"SSLv2", "SSLv3", "TLSv1", "TLSv1.1", "TLSv1.2", "TLSv1.3"}
        local suc, err = ssl.set_protocols(ops)
        if not suc then
            ngx.log(ngx.ERR, "failed to set protocols: ", err)
            return
        end
    }   
...
=== TEST 27: set_protocols - no TLSv1
--- skip_openssl: 6: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
        local ssl = require "ngx.ssl"
        local ops = {"SSLv2", "SSLv3", "TLSv1.1", "TLSv1.2", "TLSv1.3"}
        local suc, err = ssl.set_protocols(ops)
        if not ver then
            ngx.log(ngx.ERR, "failed to set protocols: ", err)
            return
...
--- error_log eval
qr/\[error\] .*? SSL_do_handshake\(\) failed .*?tlsv1 alert protocol version/
...
=== TEST 28: set_protocols - no TLSv1.1
--- skip_openssl: 6: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
        local ssl = require "ngx.ssl"
        local ops = {"SSLv2", "SSLv3", "TLSv1", "TLSv1.2", "TLSv1.3"}
        local suc, err = ssl.set_protocols(ops)
        if not ver then
            ngx.log(ngx.ERR, "failed to set protocols: ", err)
            return
...
--- error_log eval
qr/\[error\] .*? SSL_do_handshake\(\) failed/
...
=== TEST 29: set_protocols - no TLSv1.2
--- skip_openssl: 6: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
        local ssl = require "ngx.ssl"
        local ops = {"SSLv2", "SSLv3", "TLSv1", "TLSv1.1", "TLSv1.3"}
        local suc, err = ssl.set_protocols(ops)
        --if not ver then
        --    ngx.log(ngx.ERR, "failed to set protocols: ", err)
        --    return
...
=== TEST 30: set_protocols - no TLSv1.3
--- skip_openssl: 6: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
        local ssl = require "ngx.ssl"
        local ops = {"SSLv2", "SSLv3", "TLSv1", "TLSv1.1", "TLSv1.2"}
        local suc, err = ssl.set_protocols(ops)
        if not ver then
            ngx.log(ngx.ERR, "failed to set protocols: ", err)
            return
...
=== TEST 31: set_protocols - AES256-SHA
--- skip_openssl: 5: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
        local ssl = require "ngx.ssl"
        local suc, err = ssl.set_ciphers("RSA+AES256:EECDH+3DES:RSA+3DES:!MD5")
        if not suc then
            ngx.log(ngx.ERR, "failed to set protocols: ", err)
            return
        end
...
=== TEST 32: set_ciphers - no ECDHE-RSA-AES256-SHA
--- skip_openssl: 6: < 1.1.1
--- http_config
    lua_package_path "$TEST_NGINX_LUA_PACKAGE_PATH";
    ssl_client_hello_by_lua_block {
