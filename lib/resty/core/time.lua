...
long ngx_http_lua_ffi_msec(void);
...
void ngx_http_lua_ffi_parse_http_time(const unsigned char *str, size_t len,
    ...
    ngx_lua_ffi_msec = C.ngx_http_lua_ffi_msec
...
long ngx_stream_lua_ffi_msec(void);
...
void ngx_stream_lua_ffi_update_time(void);
    ...
    ngx_lua_ffi_msec = C.ngx_stream_lua_ffi_msec
...
function ngx.msec()
    return tonumber(ngx_lua_ffi_msec())
end
